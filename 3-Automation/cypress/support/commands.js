...
Cypress.Commands.add('validLogin', function () {
  cy.get('input[placeholder="Username"]').type('standard_user', { log: false })
  cy.get('input[placeholder="Password"]').type('secret_sauce', { log: false })
  cy.get('input[id="login-button"]').click()
  })

Cypress.Commands.add('invalidLogin', function () {
  cy.get('input[placeholder="Username"]').type('locket_out_user', { log: false })
  cy.get('input[placeholder="Password"]').type('secret_sauce', { log: false })
  cy.get('input[id="login-button"]').click()
  })

Cypress.Commands.add('productsMinAndMax', function () {
  const products = []
...
Cypress.Commands.add('productsMinAndMaxAddCard', function () {
  const products = []
      cy.get('.inventory_item').each(($product) => {
        const name = $product.find('.inventory_item_name').text()
        const price = $product.find('.inventory_item_price').text().replace(/\$/g, '').trim()
